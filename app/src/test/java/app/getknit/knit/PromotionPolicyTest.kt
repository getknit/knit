...
class PromotionPolicyTest {
    ...
    private val cfg = PromotionConfig(
    ...
    private fun cand(nodeId: String, rssi: Double, dwell: Long = 15_000): BlePresenceTracker.Snapshot =
        BlePresenceTracker.Snapshot(
            nodeId = nodeId, protoVersion = 1, capabilities = 0L, psm = 100, digestCue = 0,
            smoothedRssi = rssi, dwellMs = dwell, lastSeenAgoMs = 0,
        )

    private fun link(nodeId: String, rssi: Double, age: Long = 60_000) =
        PromotionPolicy.LinkSnapshot(nodeId, smoothedRssi = rssi, ageMs = age, idleMs = 0)

    @Test
    fun fillsFreeSlotsWithStrongestFirst() {
        val d = PromotionPolicy.decide(
