 ...
 * fresh on re-serve, so the frame re-floods with a full hop budget. Exactly one of [recipientId] (the
 * DM's cleartext target) / [groupId] (the group's id) is set; the broadcast room is never carried.
 ...
 * roster to push toward is read from the decoded envelope ([signed]). [groupId] bounds the per-group
 * quota. [origin] distinguishes a frame we relayed for others (`ORIGIN_RELAY`) from one we authored
