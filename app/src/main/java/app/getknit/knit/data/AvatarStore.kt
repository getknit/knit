 ...
 */
class AvatarStore(private val context: Context, private val blobs: BlobDao) {
    ...
    suspend fun saveOwnAvatar(uri: Uri): String? = withContext(Dispatchers.IO) {
        loadForCrop(uri)?.let { saveOwnAvatar(it) }
    }

    /** Extracts the (already-square) [crop] region from [source], then stores it as the own avatar. */
    suspend fun saveOwnAvatar(source: Bitmap, crop: CropRect): String? = withContext(Dispatchers.IO) {
     ...
     */
    suspend fun saveOwnAvatar(bitmap: Bitmap): String? = withContext(Dispatchers.IO) {
