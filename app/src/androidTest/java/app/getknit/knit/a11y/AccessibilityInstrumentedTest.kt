 ...
 * ```
 * or on Firebase Test Lab's API-36 device via `scripts/ftl-a11y.sh`.
 *
 ...
 * report NOT_RUN (structural checks still run in full); on an FTL physical device (`scripts/ftl-a11y.sh`)
 * they run for real.
 */
    ...
    private fun writeReport() {
        ...
        val body =
            buildString {
                ...
                appendLine(" contrast is NOT_RUN unless run on real hardware, e.g. scripts/ftl-a11y.sh]")
            }
        runCatching {
