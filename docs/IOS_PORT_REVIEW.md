...
| Blob attachments | Mostly clean | Content addressing + `AttachmentCrypto` trivial. Android *encodes* WebP (`WebpTranscode`/`AnimatedWebpMuxer`); iOS decodes WebP natively (14+) but encoding needs libwebp — either bundle it or let iOS originate JPEG/HEIC-transcoded-to-JPEG. `attachmentMime` rides sealed in `MessageContent` (never on the cleartext frame — ADR 035); **specify the cross-platform mime set** (e.g. `image/webp`, `image/jpeg`, `image/png`) so neither side sends what the other can't render. Harder since ADR 2026-09.qq2r: arbitrary **files** carry an open type set plus a sealed `attachmentName`/`attachmentSize`, so the mime set to pin is now "what both sides *render inline*", with everything else falling to the named-file bubble — which is the graceful default a port gets for free. An iOS build must also advertise `Protocol.CAP_FILES` before Android will offer a file to it. Link-preview cards (ADR 2026-09.n752) add one more MIME, `application/vnd.knit.link-preview`: a CBOR container (`LinkPreviewBlob`, own golden vectors) whose inner picture is JPEG or WebP, and `Protocol.CAP_LINK_PREVIEW` gates whether Android attaches one toward a DM/group peer |
